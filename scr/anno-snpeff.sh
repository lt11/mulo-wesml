...
### input folder
in_dir="${base_dir}/var-calls/inter-filt"
### output folder
...
for ind_f in ${vcf_files}; do
  ...
  control_id=$(echo "${file_name}" | cut -d "-" -f 3)
  ...
  snpeff_file="${out_dir}/${control_id}-tab-${tumour_id}.txt"
  echo -e "${control_id}\t${tumour_id}" > "${snpeff_file}"
